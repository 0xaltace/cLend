...
export function RiskBar({ debtUsd, projectedDebtUsd, maxBorrowUsd, liqUsd }: {
  ...
  if (liqUsd <= 0) {
    return (
      <div className="well border-dashed rounded-xl px-3 py-2.5 text-xs text-t2">
        <span className="font-bold text-t1">No borrowing power yet</span> — add collateral to unlock
        the borrow meter.
      </div>
    );
  }
  ...
  const barColor = usage < 0.6 ? "var(--pos)" : usage < 0.9 ? "var(--accent)" : "var(--neg)";
  ...
  return (
    <div>
      <div className="flex justify-between text-[11px] text-t2 mb-1">
        <span>
          Borrowed <span className="font-mono text-t1">{fmtUsd(debtUsd)}</span>
          {projected !== null && projectedDebtUsd !== debtUsd && (
            <span className="font-mono text-accent-2"> → {fmtUsd(projectedDebtUsd!)}</span>
          )}
        </span>
        <span>
          Limit <span className="font-mono text-t1">{fmtUsd(maxBorrowUsd)}</span>
        </span>
      </div>
      <div className="relative h-4 rounded-full border border-edge" style={{ background: "var(--viz-track)" }}>
        ...
        <motion.div
          ...
          style={{
            background:
              projected !== null
                ? `repeating-linear-gradient(45deg, ${barColor}, ${barColor} 6px, color-mix(in srgb, ${barColor} 60%, transparent) 6px, color-mix(in srgb, ${barColor} 60%, transparent) 11px)`
                : barColor,
          }}
        />
        {/* tick marking the CURRENT debt while a projection is shown */}
        {projected !== null && projected !== used && (
          <div className="absolute -top-1 bottom-[-2px] w-0.5 bg-t1/80" style={{ left: `${used}%` }} />
        )}
