...
const EVENT_VERBS: Record<string, { verb: string; color: string }> = {
  ...
  SupplyWithdrawn: { verb: "withdrew supply", color: "text-t2" },
  CollateralAdded: { verb: "added collateral", color: "text-accent-2" },
  CollateralWithdrawn: { verb: "removed collateral", color: "text-t2" },
  Borrowed: { verb: "borrowed", color: "text-accent" },
  Repaid: { verb: "repaid", color: "text-pos" },
  Liquidated: { verb: "was liquidated", color: "text-neg" },
  HealthCheckResolved: { verb: "health check", color: "text-t2" },
  RatesUpdated: { verb: "rates synced", color: "text-t2" },
};
          ...
          {address && (
            <button
              ...
              className={`text-[10px] font-bold px-2 py-0.5 rounded-md border transition-colors ${
                mineOnly ? "border-accent text-accent" : "border-edge text-t2 hover:border-t3"
              }`}
            >
              Mine
            </button>
          )}
          <span className="tag bg-accent-2/10 text-accent-2 hidden sm:inline">Amounts: encrypted, always</span>
          <span className="tag bg-accent-2/10 text-accent-2 sm:hidden">Encrypted</span>
        </div>
      </div>
        ...
        {filtered.map((item) => {
          ...
          return (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center gap-2 text-xs font-mono well hover:border-accent/40 rounded-lg px-3 py-2 transition-colors"
            >
              <span className="text-t3">{item.actor ? shortAddr(item.actor) : "—"}</span>
              <span className={spec.color}>{spec.verb}</span>
              <CipherValue value="" hidden chars={8} className="text-[10px]" />
              <span className="text-t3 ml-auto">{item.market}</span>
            </a>
          );
        })}
        {filtered.length === 0 && (
          <div className="text-xs text-t3 py-2">
